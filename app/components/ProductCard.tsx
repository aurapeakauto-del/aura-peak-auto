    ...
    const ProductContent = () => (
        <>
            <div className="block sm:hidden p-3">
                ...
                {isOutOfStock ? (
                    <button disabled onClick={(e) => e.stopPropagation()} className="w-full py-2 bg-gray-200 text-gray-500 cursor-not-allowed rounded text-sm">غير متوفر</button>
                ) : (
                    <button onClick={handleOpenModal} className="w-full py-2 bg-[#1a1a1a] text-white hover:bg-gray-800 rounded text-sm">إضافة للسلة</button>
                )}
            </div>
            ...
            <div className="hidden sm:block p-4">
                ...
                <div className="flex items-center justify-between mb-3">
                    <div className="flex items-baseline gap-2">
                        {product.discount ? (
                            <>
                                <span className="text-[#1a1a1a] text-lg lg:text-xl font-semibold">JD {(product.price * (1 - product.discount / 100)).toFixed(2)}</span>
                ...
                {isOutOfStock ? (
                    <button disabled onClick={(e) => e.stopPropagation()} className="w-full py-3 bg-gray-200 text-gray-500 border border-gray-200 cursor-not-allowed text-sm rounded">غير متوفر</button>
                ) : (
                    <button onClick={handleOpenModal} className="w-full py-3 bg-[#1a1a1a] text-white hover:bg-gray-800 transition-colors text-sm font-medium rounded">إضافة للسلة</button>
                )}
            </div>
        </>
    );
        ...
        <>
            <div className="bg-white border border-gray-200 hover:border-gray-400 hover:shadow-md transition-all duration-300 relative">
                {product.discount && product.discount > 0 ? (
                    <div className="absolute top-2 left-2 z-20 bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg">-{product.discount}%</div>
                ) : (
